...
import { X, Square as SquareIcon, RotateCcw, Check } from "lucide-react";
...
export type SpawnArea = { north: number; south: number; east: number; west: number };

type Props = {
  open: boolean;
  initial: SpawnArea | null;
  center?: { lat: number; lng: number };
  onClose: () => void;
  onSave: (area: SpawnArea | null) => void;
};

export function SpawnAreaDrawer({ open, initial, center, onClose, onSave }: Props) {
  const ref = useRef<HTMLDivElement | null>(null);
  const mapRef = useRef<any>(null);
  const rectRef = useRef<any>(null);
  const corner1Ref = useRef<{ lat: number; lng: number } | null>(null);
  const [hint, setHint] = useState("Tap two corners on the map to draw a square");
  const [hasArea, setHasArea] = useState(!!initial);
    ...
    loadMaps().then(() => {
      ...
      map.addListener("click", (e: any) => {
        ...
        if (!corner1Ref.current) {
          ...
          setHasArea(true);
          setHint("Looks good? Save, or tap Reset to redraw.");
        }
      });
  ...
  const reset = () => {
    ...
    corner1Ref.current = null;
    setHasArea(false);
    setHint("Tap two corners on the map to draw a square");
  };

  const save = () => {
    if (!rectRef.current) { onSave(null); onClose(); return; }
    const b = rectRef.current.getBounds();
    const ne = b.getNorthEast();
    const sw = b.getSouthWest();
    onSave({ north: ne.lat(), south: sw.lat(), east: ne.lng(), west: sw.lng() });
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-background flex flex-col">
      <div className="flex items-center justify-between px-4 h-14 border-b border-border">
        ...
        <p className="font-display font-bold text-sm">Draw spawn area</p>
        <button onClick={reset} className="h-9 w-9 rounded-full bg-card flex items-center justify-center" title="Reset"><RotateCcw className="h-4 w-4" /></button>
      </div>
      ...
      <div className="p-4 border-t border-border flex gap-2">
        ...
        <button onClick={save} disabled={!hasArea}
          className="flex-1 h-12 rounded-2xl bg-gradient-to-r from-primary to-secondary text-primary-foreground font-bold text-sm disabled:opacity-40 flex items-center justify-center gap-2">
          <Check className="h-4 w-4" /> Save area
        </button>
      </div>
    </div>
  );
